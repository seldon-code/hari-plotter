    ...
    def mean_graph(self, images: List[Graph]) -> Graph:
        ...
        for node in nodes:
            ...
            for attr, method in self.node_parameters_in_mean_graph.items():
                if method == 'mean':
                    ...
                    node_attributes[attr] = types[0]
            mean_graph.add_node(node, **node_attributes)

        # Calculate attributes for edges
        edges_set = set()
        for graph in images:
        ...
        for u, v in edges_set:
            ...
            for attr, method in self.edge_parameters_in_mean_graph.items():
                if method == 'mean':
                    influences = [graph.edges[u, v][attr]
                                  for graph in images if graph.has_edge(u, v)]
                    if influences:
                        edge_attributes[attr] = np.mean(influences)
            if edge_attributes:
                mean_graph.add_edge(u, v, **edge_attributes)
